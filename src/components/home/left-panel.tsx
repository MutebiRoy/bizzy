@tailwind base;
@tailwind components;
@tailwind utilities;

@layer base {
    :root {
        --background: 48, 8%, 88%;
        --foreground: 222.2 84% 4.9%;
        --container: 0 0 100%;
        --left-panel: 203, 32%, 10%;
        --gray-primary: 216, 20%, 95%;
        --gray-secondary: 216, 20%, 95%;
        --left-panel: 100, 100%, 100%;
        --chat-hover: 180, 5%, 96%;
        --green-primary: 167, 100%, 33%;
        --green-chat: 111, 91%, 91%;
        --card: 0 0% 100%;
        --card-foreground: 222.2 84% 4.9%;
        --popover: 0 0% 100%;
        --popover-foreground: 222.2 84% 4.9%;
        --primary: 222.2 47.4% 11.2%;
        --primary-foreground: 210 40% 98%;
        --secondary: 210 40% 96.1%;
        --secondary-foreground: 222.2 47.4% 11.2%;
        --muted: 210 40% 96.1%;
        --muted-foreground: 215.4 16.3% 46.9%;
        --accent: 210 40% 96.1%;
        --accent-foreground: 222.2 47.4% 11.2%;
        --destructive: 0 84.2% 60.2%;
        --destructive-foreground: 210 40% 98%;
        --border: 214.3 31.8% 91.4%;
        --input: 214.3 31.8% 91.4%;
        --ring: 222.2 84% 4.9%;
        --radius: 0.5rem;
    }

    .dark {
        --background: 202, 31%, 7%;
        --foreground: 210 40% 98%;
        --container: 202, 31%, 7%;
        --gray-primary: 202, 23%, 16%;
        --gray-secondary: 202, 22%, 17%;
        --left-panel: 203, 32%, 10%;
        --chat-hover: 202, 23%, 16%;
        --green-primary: 167, 100%, 33%;
        --green-secondary: 165, 100%, 39%;
        --green-chat: 169, 100%, 18%;
        --gray-tertiary: 203, 22%, 21%;
        --card: 222.2 84% 4.9%;
        --card-foreground: 210 40% 98%;
        --popover: 222.2 84% 4.9%;
        --popover-foreground: 210 40% 98%;
        --primary: 210 40% 98%;
        --primary-foreground: 222.2 47.4% 11.2%;
        --secondary: 217.2 32.6% 17.5%;
        --secondary-foreground: 210 40% 98%;
        --muted: 217.2 32.6% 17.5%;
        --muted-foreground: 215 20.2% 65.1%;
        --accent: 217.2 32.6% 17.5%;
        --accent-foreground: 210 40% 98%;
        --destructive: 0 62.8% 30.6%;
        --destructive-foreground: 210 40% 98%;
        --border: 217.2 32.6% 17.5%;
        --input: 217.2 32.6% 17.5%;
        --ring: 212.7 26.8% 83.9%;
    }

    .dark {
        --background: 202, 31%, 7%;
        --foreground: 210 40% 98%;
    }

    body {
        @apply bg-background text-foreground;
        overflow: visible;
  height: auto;
  margin: 0;
  padding: 0;
    }
    /* html,
    body,
    #__next {
        height: 100%;
        display: flex;
        flex-direction: column;
    } */
}

@layer base {
    input,
    textarea,
    select {
        @apply text-base;
    }
    * {
        @apply border-border;
    }
    body {
        @apply bg-background text-foreground;
    }
}

/* @layer components {
    ::-webkit-scrollbar {
        width: 8px;
    }
    ::-webkit-scrollbar-thumb {
        background-color: hsl(var(--gray-primary));
        border-radius: 4px;
    }
    ::-webkit-scrollbar-track {
        background-color: hsl(var(--container));
    }
} */

input,
textarea,
select {
  @apply text-base;
}

/* html,
body,
#root,
#__next {
  height: 100%;
  overflow: hidden; 
} */
/* .chat-container {
    padding-bottom: env(safe-area-inset-bottom);
    position: sticky;
    bottom: 0;
    display: flex;
    flex-direction: column;
    height: 100%;
} */

.message-input-container {
    width: 100%; /* Make sure input takes full width */
}

.app-container {
    /* A full-height container, so the main can fill remaining space */
    position: relative;
    min-height: 100vh;
    /* no scrolling here, let main handle the scroll */
  }
  
  .app-header,
  .app-footer {
    /* Typically a fixed height that you measure with offsetHeight */
    height: 70px; /* Or however tall your headers/footers are */
    left: 0;
    right: 0;
  }
  
  .app-main {
    overflow-y: auto;
    margin-top: 70px;
    margin-bottom: 70px;
    -webkit-overflow-scrolling: touch; /* for smooth iOS scrolling */
    background: brown;
  }
  .app-header {
    padding-top: env(safe-area-inset-top);
  }
  .app-footer {
    padding-bottom: env(safe-area-inset-bottom);
  }
